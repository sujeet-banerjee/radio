...
int stk500_getsync(PROGRAMMER * pgm)
{
  ...
  fprintf(stderr, "\n [type=Arduino] stk500_getsync DONE.");
  return 0;
}
...
static int stk500_cmd(PROGRAMMER * pgm, const unsigned char *cmd,
                      unsigned char *res)
{
	fprintf(stderr, "\n [type=Arduino] UNIVERSAL stk500_cmd CMD=[%2X] [%2X] [%2X] [%2X] \n",
  ...
  fprintf(stderr, "\n [type=Arduino] UNIVERSAL stk500_cmd RESP=[%2X] [%2X] [%2X] [%2X]  DONE\n",
...
{
	fprintf(stderr, "\n [type=Arduino] stk500_program_enable");
 ...
 retry:
  ...
  if (stk500_recv(pgm, buf, 1) < 0)
    exit(1);
  if (buf[0] == Resp_STK_OK) {
	  fprintf(stderr, "\n [type=Arduino] stk500_program_enable DONE.");
    return 0;
  }
                                     ...
                                     unsigned char * cmd)
{
	...
	fprintf(stderr, "\n [type=Arduino] stk500_set_extended_parms");
  unsigned char buf[16];
  int tries=0;
  int i;
 ...
 retry:
  ...
  if (buf[0] == Resp_STK_OK) {
	  ...
	  fprintf(stderr, "\n [type=Arduino] stk500_set_extended_parms DONE.");
    return 0;
  }
  else if (buf[0] == Resp_STK_NODEVICE) {
...
{
	fprintf(stderr, "\n [type=Arduino] stk500_initialize");
 ...
 retry:
  ...
  buf[buf_size-1] = Sync_CRC_EOP;

  fprintf(stderr, "\n [type=Arduino] stk500_initialize deviceCode=%2X deviceRev=%2X deviceCommn=%2X deviceParallel=%2X\n",
  		  buf[1], buf[2], buf[3], buf[4]);

  fprintf(stderr, "\n [type=Arduino] stk500_initialize PGM Params: ");
  for(int i=0; i<32 ; i++)
  {
	  // ---
	  buf[i] = 0;
  ...
  stk500_send(pgm, buf, buf_size);

  if (stk500_recv(pgm, buf, 1) < 0)
    exit(1);

  //---
  fprintf(stderr, "\n [type=Arduino] stk500_initialize PGM Resp: ");
  for(int j=0; j<32; j++)
	  fprintf(stderr, " [%2X] >", buf[j]);
  fprintf(stderr, "\n");
  //---
  ...
  if (buf[0] != Resp_STK_OK) {
    ...
    //---
    fprintf(stderr, "\n [type=Arduino] stk500_initialize PGM Params-2: ");
    for(int i=0; i<32 ; i++)
	{
	  // ---
	  buf2[i] = 0;
	  buf2[0] = Cmnd_STK_SET_DEVICE;
	  buf2[buf_size-1] = Sync_CRC_EOP;
	  // ---

	  fprintf(stderr, " [%2X] =", buf2[i]);
	}
	fprintf(stderr, "\n");
    stk500_send(pgm, buf2, buf_size);
    stk500_recv(pgm, buf2, 2);
    //---
  ...
  fprintf(stderr, "\n [type=Arduino] stk500_initialize DONE.\n");
...
static int stk500_loadaddr(PROGRAMMER * pgm, unsigned int addr)
{
	...
	fprintf(stderr, "\n [type=Arduino] stk500_loadaddr %4X\n", addr);
  unsigned char buf[16];
  int tries;
 ...
 retry:
  ...
  if (stk500_recv(pgm, buf, 1) < 0)
    exit(1);
  if (buf[0] == Resp_STK_OK) {
	  fprintf(stderr, "\n [type=Arduino] stk500_loadaddr DONE.\n");
    return 0;
  }
                              ...
                              unsigned int addr, unsigned int n_bytes)
{
	...
	fprintf(stderr, "\n [type=Arduino] stk500_paged_write baseAddr=%4X, pageSize=%4X, nBytes=%4X \n",
...
//#endif
  ...
  retry:
    ...
    buf[i++] = memtype;
    memcpy(&buf[i], &m->buf[addr - m->offset], block_size);
    i += block_size;
    buf[i++] = Sync_CRC_EOP;
  ...
  fprintf(stderr, "\n [type=Arduino] stk500_paged_write DONE. \n");
  return n_bytes;
}
                             ...
                             unsigned int addr, unsigned int n_bytes)
{
	...
	fprintf(stderr, "\n [type=Arduino] stk500_paged_load  addr=%4X \n", addr);
  unsigned char buf[16];
  int memtype;
  int a_div;
  int tries;
...
{
	fprintf(stderr, "\n [type=Arduino] stk500_set_vtarget \n");
...
{
	fprintf(stderr, "\n [type=Arduino] stk500_set_sck_period \n");
  int dur;
  double min, max;
...
static int stk500_setparm(PROGRAMMER * pgm, unsigned parm, unsigned value)
{
	fprintf(stderr, "\n [type=Arduino] stk500_setparm param=%2X , value=%2X", parm, value);
  unsigned char buf[16];
  int tries = 0;
...
static void stk500_print_parms1(PROGRAMMER * pgm, const char * p)
{
	fprintf(stderr, "\n [type=Arduino] stk500_print_parms1 \n");
