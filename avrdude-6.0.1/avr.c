             ...
             AVRPART * v)
{
	...
	fprintf(stderr, "\n [type=Arduino] avr_read; is V Null?? V=%d", v);
  ...
  if ((p->flags & AVRPART_HAS_TPI) && mem->page_size != 0 &&
	  ...
	  fprintf(stderr, "\n [type=Arduino] avr_read Read pages through post-increment");
  ...
  if (pgm->paged_load != NULL && mem->page_size != 0) {
    ...
    /* quickly scan number of pages to be read, first */
    ...
    fprintf(stderr, "\n [type=Arduino] avr_read Number of pages to read = %d (dec), readBase address = %4X (Hex) \n",
    		npages, pageStartAddr);

    for (pageaddr = pageStartAddr, failure = 0, nread = 0;
      ...
      for (i = pageaddr, need_read = 0;
           ...
           i++)
        if (vmem == NULL /* no verify, read everything */ ||
            (vmem->tags[i-pageStartAddr] & TAG_ALLOCATED) != 0 /* verify, do only
                                                    read pages that
                                                    are needed in
                                                    input file */) {
          need_read = 1;
              ...
              int auto_erase)
{
  ...
  //Sujeet
  int pageStartAddr = PG_ST_ADDR;
  //pageStartAddr = 0x00;
  //---
  fprintf(stderr, "\n [type=Arduino] avr_write UU_pageStart=%4X \n", pageStartAddr);
  ...
  fprintf(stderr, "\n [type=Arduino] avr_write UU_pageStart=%4X msgSize=%d (dec), requestedSize = %d (dec)\n",
		  pageStartAddr, m->size, size);
  ...
  if ((p->flags & AVRPART_HAS_TPI) && m->page_size != 0 &&
	  ...
	  fprintf(stderr, "\n [type=Arduino] avr_write DOING THE TPI STUFF!!");
  ...
  if (pgm->paged_write != NULL && m->page_size != 0) {
	  ...
	  fprintf(stderr, "\n [type=Arduino] avr_write DOING THE PAGE-MODE-WRITE!!");
    ...
    fprintf(stderr, "\n [type=Arduino] avr_write Calculating the number of pages to be written \n");
    ...
    fprintf(stderr, "\n [type=Arduino] avr_write Number of pages to write = %d (dec), writeBase-address = %4X (Hex) \n",
        		npages, pageStartAddr);

    for (pageaddr = pageStartAddr, failure = 0, nwritten = 0;
         !failure && pageaddr < (wsize + pageStartAddr);
         pageaddr += m->page_size) {
      /* check whether this page must be written to */
      for (i = pageaddr, need_write = 0;
           i < pageaddr + m->page_size;
           i++)
        if ((m->tags[i-pageStartAddr] & TAG_ALLOCATED) != 0) {
          need_write = 1;
          break;
        }
...
int avr_verify(AVRPART * p, AVRPART * v, char * memtype, int size)
{
  ...
  fprintf(stderr, "\n [type=Arduino] avr_verify MATCHING memTypeP=%s at offset=%4X (Hex), "
		  "with memTypeV=%s at offset=%4x (Hex).", a->desc, a->offset, b->desc, b->offset);
  ...
  for (i=0; i<size; i++) {
    if ((b->tags[i] & TAG_ALLOCATED) != 0 &&
        buf1[i + a->offset] != buf2[i]) {
      fprintf(stderr,
              "%s: verification error, first mismatch at Flash byte 0x%04x with Program byte 0x%04x\n"
              "%s0x%02x != 0x%02x\n",
              progname, i + a->offset, i,
              progbuf, buf1[i+ a->offset], buf2[i]);
      return -1;
    }
  }
