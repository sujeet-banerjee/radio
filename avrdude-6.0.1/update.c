...
{
	fprintf(stderr, "\n [type=Arduino] do_op opCode=%2d, fileName=%s, memType=%s",
			upd->op, upd->filename, upd->memtype);
  struct avrpart * v;
  AVRMEM * mem;
  int size, vsize;
  int rc;
  ...
  if (upd->op == DEVICE_READ) {
	  ...
	  fprintf(stderr, "\n [type=Arduino] do_op !!DEVICE_READ!!");
    ...
    if (rc < 0) {
	  ...
	  fprintf(stderr, "\n [type=Arduino] do_op !!DEVICE_WRITE!!");
    ...
    if (quell_progress < 2) {
	  ...
	  fprintf(stderr, "\n [type=Arduino] do_op !!DEVICE_VERIFY!!");
    ...
    if (rc < 0) {
      fprintf(stderr, "\n\n !! ------------------ %s: verification error; content mismatch ---------- !! \n\n",
              progname);
      pgm->err_led(pgm, ON);
      return -1;
    }
