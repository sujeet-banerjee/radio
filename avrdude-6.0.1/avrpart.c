...
AVRMEM * avr_locate_mem(AVRPART * p, char * desc)
{
  ...
  for (ln=lfirst(p->mem); ln; ln=lnext(ln)) {
    ...
    if ((*(m->desc) == 'e' && *(m->desc + 1) == 'e') || (*(m->desc) == 'f'))
    {
    	fprintf(stderr, "\n [type=Arduino] avr_locate_mem Checking for "
    		" Mem=%s, mem.pageSize=%d, mem.offset=%4X Hex, mem.size=%d, mm.tags=%s, mem.buf=",
    		m->desc, m->page_size, m->offset, m->size, m->tags);
    	fprintf(stderr, "\n");
    }
  ...
  if (matches == 1)
  {
	  fprintf(stderr, "\n [type=Arduino] avr_locate_mem Matched  Mem=%s \n",
	      		desc);
    return match;
  }
